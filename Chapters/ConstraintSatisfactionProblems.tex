...
With the restiction on given parameters to the constraint.
...
The Eigth Queens Puzzle is really good suited as an example problem domain for constraint satisfaction problems because it is easy to understand and can also easily be scaled up to increase complexity for a solver. By generalizing the problem from a fixed $8 \times 8$ grid size to an $n \times n$ grid with $n$ queens, the problem remains the same in principle, but gets way harder to solve.
...
% I think this should be some bib entry?
This model is from a past MiniZinc challenge by Reza Rafeh July 2005 and Peter Stuckey September 30 2006. See \url{https://github.com/MiniZinc/minizinc-benchmarks/blob/master/queens/queens.mzn} for reference.
